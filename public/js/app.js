...
function getPlayersDisplay(players){
	let html = '<div class="row"><div class="col-xs-5ths">Name</div><div class="col-xs-5ths">Status</div><div class="col-xs-5ths">Preferred Position</div><div class="col-xs-5ths">Update</div><div class="col-xs-5ths">Delete</div></div>';
	Object.keys(players).forEach(function(key){
...
function getVisualLayersByRoster(roster){
	...
	setRosterName(roster.name+' - '+applicationState.formations[roster.formationId]);
	...
	$('.js-player-positions').append(draggableHtml);
	$('.js-main-visual').append(visualHtml);
	...
	for(let k = 0; k <= playerPositions.length; k++) {
		...
		$(`ul#${layer} li[data-position="${k+1}"]`)
			.empty()
			.append(`<div class="js-player-filled player-filled" data-playerId="${player.id}">${player.name}</div>`);
...
function getPlayerItem(player){
	return `<div class="js-player-filled player-filled" data-playerId="${player.id}">${player.name}</div>`;
}
...
function getVisualLayersByFormation(formation){
	...
	for(let j=0; j < Object.keys(applicationState.players).length; j++)
		draggableHtml+=`<li class="js-position position">Bench</li>`;
